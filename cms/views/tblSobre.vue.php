...
<thead style="display:block; border-bottom: solid 1px black;">
    <tr>
        ...
        <th style="padding:7px; width: 113px; border-right:solid 1px black; display:inline-block;">Criado:</th>
...
<tbody>
    <tr style="border-bottom:solid 1px;" v-for="(artigo,index) in artigos">
        <td style="padding:2px; width: 302px; display:inline-block; text-align: center;">{{artigo.titulo}}</td>
        <td style="padding:2px; width: 130px; display:inline-block; text-align: center;">{{artigo.dtCriacao}}</td>
        <td style="padding:2px; width: 70px; display:inline-block; text-align: center;">
            <img style="display:block; margin-left:auto; margin-right:auto;" v-if="artigo.estado=='V'" @click="activeArtigo(index)" src='./img/Accept-icon.png'>
...
Vue.component('tbl-artigos', {
  ...
  methods:{
      ...
      activeArtigo:function(index){
          var artigo = this.artigos[index];
          var novoEstado = (artigo.estado=="V") ? "F":"V";
          
          this.$http.get("./app/sobre.php?"+
          `action=AlterarEstado&idArtigo=${artigo.id}&Estado=${novoEstado}`)
            .then(function(response){
...
<div id="appTbl">
<tbl-artigos :artigos="artigos" @emit-update="update"></tbl-artigos>
</div>
<script>
var appTbl = new Vue({
    el:"#appTbl",
    ...
    methods:{
      update:function(){
        this.$http.get('./app/sobre.php?action=list').then(function(response) {
